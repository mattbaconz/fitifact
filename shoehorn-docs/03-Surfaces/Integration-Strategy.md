...
Shoehorn must not become six separate products with six separate compatibility implementations.
...
iOS Share Extension ───────┼──> Shoehorn Core
CLI ───────────────────────┤
Web SDK ───────────────────┤
REST API ──────────────────┘
```
...
- bridge launches or messages the native Shoehorn process;
...
- small bridge invokes native Shoehorn;
...
```text
Share -> Shoehorn -> target -> adapt -> share onward
```
...
The file never needs to reach Shoehorn Cloud unless managed execution is explicitly chosen.
