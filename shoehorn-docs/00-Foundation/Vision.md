...
Shoehorn becomes the compatibility layer between **arbitrary files** and **arbitrary destinations**.
...
destination understands its own constraints
   ↓
Shoehorn adapts locally or via managed worker
   ↓
validated output
   ↓
accepted
...
Eventually, users should rarely need to know Shoehorn exists.
...
Shoehorn defines **file adaptation** as a first-class primitive:
...
Products integrate Shoehorn so users stop seeing preventable upload errors.

### 3. Compatibility infrastructure
Shoehorn operates a continuously tested compatibility registry and managed compute fabric.

## Anti-vision

Shoehorn fails if it becomes:
