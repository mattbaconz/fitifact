...
`video.mov` is HEVC/AAC, 41.8 MB. Target needs MP4/H.264 <=25 MB. Fitifact selectively transcodes video, preserves valid audio, fits size, validates.

### “It is already MP4. Why does it fail?”
MP4 container contains unsupported HEVC. Fitifact diagnoses container vs. codec and changes only the incompatible stream.

### Assignment file too large
Target `< 2 MB`. Fitifact tries lossless/low-impact reductions first, then controlled quality reduction, stopping below a safe margin. PDF is likely phase 2.
