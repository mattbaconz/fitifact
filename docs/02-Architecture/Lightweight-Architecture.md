...
Fitifact should be **small, dormant when unused, and lazy about loading heavy capability**.
...
Fitifact does not need to own the encoder. It needs a provider capable of producing the target state.
...
Fitifact's UI is too small to justify a permanent bundled browser runtime without measured benefits.
...
> Fitifact only loads the parts it needs.

> If your file already fits, Fitifact changes nothing.
